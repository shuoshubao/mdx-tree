...
import { Button, Card, ConfigProvider, FloatButton, Layout, Result, Skeleton, Tree, theme } from 'antd'
...
const { code } = window.data

const files = data.files.filter(v => v.name.endsWith('.md'))
...
const App = () => {
  ...
  const [isFile, setIsFile] = useState(location.pathname.endsWith('.md'))
    ...
    <>
      ...
      <Content style={{ padding: token.paddingContentVertical, minHeight: '100vh', overflowY: 'auto' }}>
        <Skeleton loading={isFile}>
          <Card title={location.pathname.slice(1) + '.md'}>
