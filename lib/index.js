...
import { Button, Card, ConfigProvider, FloatButton, Layout, Result, Skeleton, Tree, Typography, theme } from 'antd'
...
import { slugify, MarkdownItHighlight } from './markdown'
import './index.css'
...
const App = () => {
  ...
  const fetchData = async () => {
    ...
    updateSelectedKeys()
    setTocData(TocData)
    setSelectedKeys([pathname.slice(1) + '.md'])
  }
    ...
    <>
      ...
      <Content style={{ padding: token.paddingContentVertical, minHeight: '100vh', overflowY: 'auto' }}>
        <Skeleton loading={isFile}>
          <Card title={window.location.pathname.slice(1) + '.md'}>
